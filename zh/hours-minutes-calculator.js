(() => {
  ...
  if (!durationList || !addDurationButton || !resetButton || !copyButton || !resultHmm || !resultMinutes || !resultDecimal) {
    return;
  }
  ...
  function rowMarkup(duration, index) {
    ...
    const signText = isAdd ? "相加 +" : "相减 −";
    const nextSign = isAdd ? "相减" : "相加";
    const rowNumber = index + 1;

    return `
      <div class="duration-row" data-duration-id="${duration.id}">
        <div class="duration-row-header">
          <span class="duration-row-label">时长 ${rowNumber}</span>
          <div class="duration-row-actions">
            <button class="duration-sign${isAdd ? " is-add" : " is-subtract"}" type="button" data-action="toggle-sign" data-id="${duration.id}" aria-label="把第 ${rowNumber} 段时长改为${nextSign}">${signText}</button>
            <button class="duration-remove" type="button" data-action="remove" data-id="${duration.id}" aria-label="删除第 ${rowNumber} 段时长">删除</button>
          </div>
        </div>
        <div class="duration-row-fields">
          <div class="duration-field">
            <label class="duration-field-label" for="duration-${duration.id}-hours">小时</label>
            <span class="duration-stepper">
              <button type="button" data-action="step" data-id="${duration.id}" data-field="hours" data-delta="-1" aria-label="第 ${rowNumber} 段减少一小时">−</button>
              <input id="duration-${duration.id}-hours" type="number" inputmode="numeric" min="0" max="${MAX_VALUE}" step="1" value="${duration.hours}" data-id="${duration.id}" data-field="hours" aria-label="第 ${rowNumber} 段的小时数">
              <button type="button" data-action="step" data-id="${duration.id}" data-field="hours" data-delta="1" aria-label="第 ${rowNumber} 段增加一小时">+</button>
            </span>
          </div>
          <div class="duration-field">
            <label class="duration-field-label" for="duration-${duration.id}-minutes">分钟</label>
            <span class="duration-stepper">
              <button type="button" data-action="step" data-id="${duration.id}" data-field="minutes" data-delta="-5" aria-label="第 ${rowNumber} 段减少五分钟">−</button>
              <input id="duration-${duration.id}-minutes" type="number" inputmode="numeric" min="0" max="${MAX_VALUE}" step="1" value="${duration.minutes}" data-id="${duration.id}" data-field="minutes" aria-label="第 ${rowNumber} 段的分钟数">
              <button type="button" data-action="step" data-id="${duration.id}" data-field="minutes" data-delta="5" aria-label="第 ${rowNumber} 段增加五分钟">+</button>
            </span>
          </div>
        </div>
      </div>`;
  ...
  copyButton.addEventListener("click", async () => {
    const minutes = totalMinutes();
    const text = `${formatHoursMinutes(minutes)}｜总计 ${minutes} 分钟｜${(minutes / 60).toFixed(2)} 十进制小时`;
